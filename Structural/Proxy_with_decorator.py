...
def proxy(cls):
    class Wrapper:
        def __init__(self, *args):
            self.__real_subject = cls(*args)

        def __getattr__(self, name):  # При извлечении атрибутов
            print("При кожнову виклику make_person, що там робимо...")
            return getattr(self.__real_subject, name)
...
if __name__ == "__main__":
    ...
    unique_states = ['КС852', 'КУ932', 'ММ345', 'ВМ375', 'ЦМ445', 'ЬМ375', 'ЙЙ375',
                     'ЯХ375']  # унікальний параметр окремого обєкта ( серія та номер паспорта)
